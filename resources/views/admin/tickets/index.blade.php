                ...
                <tbody>
                    @forelse($tickets as $ticket)
                        <tr>
                            ...
                            <td class="text-start">
                                <a href="{{ route('admin.tickets.show', $ticket->id) }}" class="fw-semibold text-decoration-none">
                            ...
                            <td class="text-start">
                                @php
                                    $user = $ticket->siteUser;
                                    $borderColor = $user?->activeSubscription?->plan?->border_color ?? '#ddd';
                                    $profileImg = $user?->profile_img ? asset('storage/' . $user->profile_img) : null;
                                    $username = $user?->username ?? 'N/A';
                                    $initial = strtoupper(substr($username, 0, 1));
                                    $planTitle = $user?->activeSubscription?->plan?->title ?? 'No active plan';
                                @endphp
                                ...
                                @else
                                    <span title="{{ $planTitle }}" class="me-2 rounded-circle d-inline-flex justify-content-center align-items-center text-white"
                            ...
                            <td class="align-middle">
                                @if(is_null($ticket->assigned_to))
                                    <a href="{{ route('admin.tickets.set_assign', $ticket->id) }}" class="btn btn-sm btn-primary">
                                ...
                                @else
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        <img src="{{ $assignee->img_url ? asset($assignee->img_url) : asset('images/default-user.png') }}"
                                             alt="{{ $assignee->user }}" class="rounded-circle" width="40" height="40">
                                        <div class="text-start">
                                            <div class="fw-bold">{{ $assignee->user ?? 'User #' . $assignee->id }}</div>
                                            <div class="text-muted small">{{ $assignee->panel ?? 'App User' }}</div>
                            ...
                            {{-- Actions --}}
                            <td class="text-end">
                                <a href="{{ route('admin.tickets.show', $ticket->id) }}" class="btn btn-sm btn-info" title="View"><i class="fa fa-eye"></i></a>
                                <a href="{{ route('admin.tickets.edit', $ticket->id) }}" class="btn btn-sm btn-warning" title="Edit"><i class="fa fa-edit"></i></a>
                                <form action="{{ route('admin.tickets.destroy', $ticket->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this ticket?')">
                                    @csrf
                                    @method('DELETE')
