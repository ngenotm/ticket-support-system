...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="fw-semibold mb-0">✏️ Edit Ticket</h4>
        <span class="badge bg-secondary">{{ $ticket->ticket_track_id }}</span>
    </div>
    <hr>

    <form action="{{ route('admin.tickets.update', ['id' => $ticket->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row g-3">
                ...
                <div class="mt-3">
                    ...
                    <div class="d-flex flex-wrap gap-3">
                        @foreach($ticket->attachments as $file)
                            ...
                            @if($isImage)
                            <div class="border p-2 rounded bg-light attachment-thumb text-center" style="width:120px;">
                                <a href="{{ $fileUrl }}" target="_blank">
                                    <img src="{{ $fileUrl }}" alt="{{ $file->file_name }}" class="img-fluid rounded mb-1" style="max-height:100px;">
                                </a>
            ...
            <div class="col-12 text-end mt-4">
                <a href="{{ route('admin.tickets.index') }}" class="btn btn-light border me-2">Cancel</a>
...
$(document).ready(function () {
    ...
    function loadServices(subcatId) {
        ...
        $.ajax({
            ...
            error: function() { alert('Failed to load services.'); }
        });
    }

    $('#cat_id').change(function () { loadSubcategories($(this).val()); });
    $('#subcat_id').change(function () { loadServices($(this).val()); });
...
@push('styles')
<style>
.attachment-thumb:hover { transform: scale(1.03); transition: 0.2s ease; }
</style>
@endpush
